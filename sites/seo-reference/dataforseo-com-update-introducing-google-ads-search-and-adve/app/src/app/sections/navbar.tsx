...
export default function Navbar({ tileData = tileDataContent, listRowData = listRowDataContent, listRow2Data = ListRow2_data } = {}) {
  return (
    <header className="box-content border-b border-solid border-b-clr-0 block relative z-1000 bg-background">
      <div className="box-content block text-background text-[0.75rem] bg-color-002">
        <div className="box-content block max-w-320 px-[2.4rem] max-md:px-4 md:max-lg:px-[1.4375rem] 2xl:mx-68 2xl:px-12">
          <div className="box-content h-10 min-h-10 flex justify-between items-center max-lg:justify-center">
            <div className="box-content block">
              <a className="box-content inline-block py-1 px-5 rounded-[20px] text-color-002 text-[0.8125rem] bg-primary cursor-pointer whitespace-nowrap max-md:px-2.5 hover:transform-[matrix(1.01967,0,0,1.01967,0,0)] focus:transform-[matrix(1.00032,0,0,1.00032,0,0)]" data-component="link" href="/apis/ai-optimization-api/llm-mentions-api">
      ...
      <div className="box-content block">
        <div className="box-content flex relative max-w-320 py-4 px-[2.4rem] justify-between items-center max-md:px-4 md:max-lg:px-[1.4375rem] 2xl:mx-68 2xl:px-12">
          ...
          <nav className="box-content w-[36.1875rem] block mr-8 ml-[251.3px] transform-[none] max-md:w-[20.4375rem] max-lg:fixed max-lg:top-25.5 max-lg:bottom-0 max-lg:left-0 max-lg:z-1 max-lg:invisible max-lg:min-w-0 max-lg:p-6 max-lg:shrink-0 max-lg:overflow-x-hidden max-lg:overflow-y-auto max-lg:bg-background max-md:transform-[matrix(1,0,0,1,-375,0)] max-lg:mx-0 md:max-lg:w-180 md:max-lg:transform-[matrix(1,0,0,1,-768,0)] 2xl:ml-[328.1px]" data-component="nav">
            ...
            <div className="box-content hidden max-lg:h-90.5 max-lg:border-t max-lg:border-solid max-lg:border-t-clr-0 max-lg:block max-lg:mt-8 max-lg:pt-8">
              <div className="box-content flex mb-6 flex-col gap-3 max-lg:h-[3.1875rem]">
                <a className="box-content flex relative z-1 min-w-0 py-3.5 px-[2.1875rem] rounded-[3px] justify-center items-center gap-[0.4375rem] overflow-hidden text-color-005 font-extrabold uppercase bg-primary cursor-pointer max-lg:h-[1.4375rem]" href="https://app.dataforseo.com/register">
                  Try for free
                </a>
                {" "}
              </div>
          ...
          <div className="box-content flex items-center gap-4 max-lg:hidden">
            <a className="box-content h-[1.4375rem] border-2 border-solid border-primary block relative z-1 py-3 px-6 rounded-sm overflow-hidden text-primary font-bold cursor-pointer whitespace-nowrap before:content-[''] before:block before:absolute before:inset-y-0 before:inset-x-[59.5px] before:-z-1 before:w-0 before:h-[2.9375rem] before:bg-primary before: before:origin-[0px_23.5px] max-lg:before:left-1/2 max-lg:before:h-full max-lg:before:right-auto max-lg:before:bottom-auto max-lg:before:transform-[none] max-lg:before:origin-[initial] hover:text-clr-8 hover:outline-clr-8 hover:[text-decoration-color:var(--clr-8)] focus:text-clr-11 focus:outline-clr-11 focus:[text-decoration-color:var(--clr-11)]" data-component="link" href="https://app.dataforseo.com/register">
              Try for free
            </a>
          </div>
          {" "}
          <button className="w-10 h-10 hidden relative z-2 min-w-0 flex-col justify-center items-center shrink-0 text-color-002 [font-family:Arial] text-[0.8125rem] font-normal leading-[0.9375rem] text-center cursor-pointer max-lg:flex max-md:ml-[85.1px] md:max-lg:ml-[29rem]">
            <span className="box-content w-6 h-0.5 block absolute top-3 bg-color-005 max-lg:min-w-0" />
            {" "}
            <span className="box-content w-6 h-0.5 block absolute bg-color-005 max-lg:min-w-0" />
            {" "}
            <span className="box-content w-6 h-0.5 block absolute top-6.5 bg-color-005 max-lg:min-w-0" />
            {" "}
          </button>
          {" "}
        </div>
