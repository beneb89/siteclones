...
export default function MediaLink2({ d, styles }: { d: MediaLink2Data; styles: MediaLink2Styles }) {
  return (
    <button className={cn("w-[19.9375rem] border-b border-solid border-b-color-003 flex relative py-6 px-8 rounded-[7px] justify-center items-center shrink-0 gap-5 text-muted text-sm font-medium leading-[1.375rem] text-left cursor-pointer max-lg:border-r max-lg:border-r-color-003 max-lg:min-w-50 max-lg:py-4 max-lg:px-6 max-lg:border-b-[0] max-lg:border-initial max-lg:border-b-[initial] before:content-[''] before:block before:absolute before:top-[1.4375rem] before:right-[19.6875rem] before:bottom-0 before:left-0 before:h-11.5 before:bg-primary before:origin-[2px_23px] max-lg:before:top-18 max-lg:before:right-0 max-lg:before:h-1 hover:border-b-background hover:border-l-clr-37 hover:border-r-clr-37 hover:border-t-clr-37 focus:outline-clr-55", styles.className)} data-component="button">
      ...
      <div className="flex flex-col flex-1 gap-1 focus:outline-muted">
        <span className={cn("block text-color-001 text-lg font-bold leading-[1.8125rem] focus:outline-color-001", styles.className4)}>
          {d.label2}
        </span>
        {" "}
      </div>
