...
export default function MediaCard({ d, styles }: { d: MediaCardData; styles: MediaCardStyles }) {
  return (
    <div className="box-content flex min-w-0 flex-col gap-3 overflow-x-hidden overflow-y-auto [font-family:Raleway,_sans-serif,_sans-serif]">
      <div className={cn("h-75 min-h-75 block relative rounded-[10px] flex-1 overflow-hidden text-center bg-color-026 w-full", styles.className)}>
        ...
        <div className="w-[22.5625rem] h-75 flex absolute top-0 left-0 z-4 opacity-0 p-4 flex-col justify-center items-center gap-2 bg-color-026">
          <picture className="w-20 h-20 block max-md:w-16.5">
            <source className="inline" srcSet={d.srcSet2} type="image/webp" />
            <img className={cn("block max-w-full rounded-[10px] overflow-clip object-contain align-bottom w-20 h-20", styles.className3)} data-component="image" alt="Hover icon" src={d.imgSrc2} />
          </picture>
