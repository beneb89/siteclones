...
export default function MediaCard({ d, styles }: { d: MediaCardData; styles: MediaCardStyles }) {
  return (
    <div className="box-content flex min-w-0 flex-col gap-3 overflow-x-hidden overflow-y-auto [font-family:Raleway,_sans-serif,_sans-serif]">
      <div className={cn("h-75 min-h-75 block relative rounded-[10px] flex-1 overflow-hidden text-background text-center bg-color-015 w-full md:max-lg:h-[369.1px]", styles.className)}>
        ...
        <div className="w-[34.475rem] h-75 flex absolute top-0 left-0 z-4 opacity-0 p-4 flex-col justify-center items-center gap-2 bg-color-015">
          <a className="border border-solid border-foreground flex py-[0.5625rem] px-4.5 rounded-[40px] justify-center items-center text-foreground font-semibold leading-[1.125rem] bg-background cursor-pointer max-md:leading-[1rem] md:max-lg:leading-[1.0625rem]" data-component={d.kind} href="/seminarangebot/agilitaet-scrum" rel="noopener" target="_self">
            {d.label}
          </a>
        </div>
      </div>
