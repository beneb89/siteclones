...
export default function MediaTile({ d, styles }: { d: MediaTileData; styles: MediaTileStyles }) {
  return (
    <div className="block relative mx-3">
      <a className="block max-w-87.5 rounded-2xl underline bg-foreground cursor-pointer max-md:max-w-57.5" data-component={d.kind} href={d.href}>
        {" "}
        <div className="h-[14.5625rem] block relative rounded-tl-2xl rounded-tr-2xl overflow-hidden max-md:h-[9.5625rem]">
          <img className="w-full h-[14.5625rem] block max-w-full rounded-tl-2xl rounded-tr-2xl overflow-clip object-cover max-md:h-[9.5625rem]" data-component={d.kind2} alt={d.alt} src={d.imgSrc} />
          {" "}
