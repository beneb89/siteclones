...
export default function MediaTile({ d, styles }: { d: MediaTileData; styles: MediaTileStyles }) {
  return (
    <div className={cn("flex items-center shrink-0", styles.className)}>
      <a className="h-7 flex relative px-5 items-center text-muted-foreground [font-family:'Martina_Plantijn',_Georgia,_serif] text-lg italic leading-7 [font-feature-settings:'calt',_'liga',_'onum'] cursor-pointer" data-component="link" href={d.href}>
        <svg className="block absolute top-3.5 left-2 opacity-0 min-w-0 overflow-hidden align-middle [translate:0px_-50%] [scale:0.5] h-2.5 w-auto" aria-hidden="true" fill="currentColor" viewBox="0 0 201.75 208.5" xmlns="http://www.w3.org/2000/svg">
      ...
      <span className="block text-border" aria-hidden="true">
        |
      </span>
    </div>
  );
