...
export default function MediaCard3({ d, styles }: { d: MediaCard3Data; styles: MediaCard3Styles }) {
  return (
    <div className="w-77 border-l border-solid border-l-border block px-4 max-md:w-67.5 max-lg:shrink-0 max-lg:border-l-[0] max-lg:border-initial max-lg:border-l-[initial] max-lg:px-0 md:max-lg:w-[34.5625rem]">
      <a className={cn("flex relative flex-col overflow-hidden cursor-pointer aspect-[3/5] max-lg:aspect-[3/4]", styles.className)} data-component="link" href="/insights/12-neuerungen-von-openai">
        ...
        <div className={cn("hidden relative z-19 min-w-0 items-center flex-1 overflow-hidden max-lg:flex", styles.className30)}>
          <img className={cn("block max-w-full overflow-clip object-contain align-middle max-h-full max-md:h-35.5 md:max-lg:h-72.5", styles.className31)} alt={d.alt} src={d.imgSrc} />
        </div>
        <div className={cn("flex relative z-19 items-center flex-1 overflow-hidden max-lg:hidden", styles.className32)}>
          <img className="w-full h-[17.1875rem] block max-w-full overflow-clip object-contain align-middle max-h-full" data-component="image" alt={d.alt2} src={d.imgSrc2} />
        </div>
        <div className={cn("h-[343.7px] block absolute bottom-0 inset-x-0 z-15 min-w-0 md:max-lg:h-[34.5625rem]", styles.className33)} style={d.style} />
        <div className="flex relative z-25 pt-[0.575rem] pb-3.5 px-3.5 flex-col gap-1.5 max-md:pt-[0.5625rem] max-md:pb-[13.5px] max-md:px-[13.5px] md:max-lg:pt-[1.15rem] md:max-lg:pb-[1.725rem] md:max-lg:px-[1.725rem]">
          <h3 className={cn("block font-semibold text-pretty", styles.className34)} data-component="heading" lang="de">
            {d.title}
          </h3>
          <p className={cn("block [font-family:'Martina_Plantijn',_Georgia,_serif] text-pretty [font-feature-settings:'calt',_'liga',_'onum']", styles.className35)} lang="de">
            {d.description}
          </p>
        </div>
        <div className={cn("h-full block absolute top-0 inset-x-0 z-18 min-w-0 p-[0.2875rem] pointer-events-none md:max-lg:p-[0.575rem]", styles.className36)}>
          <div className={cn("border border-solid block pointer-events-none h-full w-full", styles.className37)} />
        </div>
      </a>
    </div>
  );
